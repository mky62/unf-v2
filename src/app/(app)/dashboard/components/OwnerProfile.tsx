...
function SocialLinksDisplay({ entries }: { entries: [string, unknown][] }) {
            ...
            <div className="flex flex-wrap gap-3">
                {entries.map(([k, v]) => (
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="group flex items-center gap-2 rounded-xl border border-border/60 bg-card/50 px-4 py-2 text-sm font-medium shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/50 hover:bg-primary/5 hover:shadow-md"
                    >
                        <span className="capitalize text-foreground/80 group-hover:text-primary transition-colors">{k}</span>
                        <svg
                            className="size-3.5 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-primary"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth={2}
...
export default function OwnerProfile({ user }: { user: ProfileData }) {
    ...
    return (
        <>
            ...
            {/* Social links block (view mode) */}
            {!editing && <SocialLinksDisplay entries={entries} />}
        </>
    );
}
